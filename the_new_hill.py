...
def weird_comparison(gene_names,
    ...
    for gene_name in gene_names:
        ...
        for record in alignments.loc[gene_name]:
            ...
            base_count_list.loc[record.id] = [np.quantile(base_count, .5), # Median occurence proportion of sequences per base site where there is none for the present record - expected low for low quality fit
                                          np.quantile(antibase_count, .5)] # Median occurence proportion of sequences per base site where there is none for the present record - expected high for low quality fit
